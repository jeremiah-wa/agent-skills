...
> what does not" while that is still being settled. Once it stops changing it
> becomes the repo's `README.md` and this file goes away.

---

## Mission

This repo is the documented specification of how its owner engineers. The skills
and workflows are the current compilation of that practice; the specification is
...
rendering of them, the way the practice happens to execute this year. The test is
literal: delete everything but `docs/` and the repo rebuilds from its own
description (see [`BUILD.md`](BUILD.md)). Anything that lives only in a prompt is
not yet specified, and specifying it is the point.

Three drives, one spine:

- **Own the practice, do not inherit it.** Every skill is authored from a blank
  page so the judgement in it is the owner's, not absorbed wholesale from someone
...
- **Write it down to understand it.** Forcing the practice onto the page is how
  the owner keeps understanding it and keeps it coherent. The recreation test
  proves none of it is tacit.
- **Grow it with agents.** The system is meant to be extended, refactored, and
  rebuilt by agents, so it is specified to the standard an agent needs to
  regenerate it faithfully ([ADR-0011](decisions/0011-docs-must-reconstruct-the-repo.md)).
...
from: at first the owner's own practice, later also external skills the owner has
used and judged good. Two gates hold at every stage: **examined use** (nothing
enters that the owner has not used and judged) and **conformance** (nothing enters
as a derivative copy; prior art is re-authored to the house standard and voice,
per [ADR-0005](decisions/0005-author-from-a-blank-page.md)). One invariant never
moves: every skill here is one its owner has an opinion about.
...
- A workflow assembled from those pieces looks portable and is not. `ship` names
  ...
  it was written on and nowhere else. Neither install channel reported a problem.
...
authored by and for its owner, installable on any machine in one command.
...
- **Self-specifying.** The `docs/` set is complete enough to reconstruct the repo
  without its code: delete everything else and rebuild. That also makes the repo a
  subject for the workflows it contains, not just their source
  ([ADR-0011](decisions/0011-docs-must-reconstruct-the-repo.md)).

## In scope

- Skills the owner **actually uses**, evidenced by usage rather than assumed.
  Measured across 98 transcripts: `grilling` and `grill-me` (17 invocations),
  `implement` (8), `code-review` (3), `tdd` (1), `handoff` (1).
...
- **Skills nobody uses.** 14 of the 24 skills in the third-party collection this
  replaces have never been invoked once, including its four largest. They are not
  reproduced here.
...
- **General-purpose or shareable tooling.** This is one person's practice. It may
  become public ([ADR-0007](decisions/0007-private-until-switchover.md)), but it
  is not written for an audience.
...
| `tdd` + `committing` | What `ship`'s implementer invokes. Writing these is what makes `ship` installable anywhere |
...
Then `ship` declares `dependencies: ["tdd", "committing", "code-review"]` (the
third resolved by [ADR-0010](decisions/0010-extract-the-review-baseline-into-a-library-skill.md)).

## Non-goals for v1

- A bundle plugin for one-command install of everything. Undecided, and premature
  at three plugins.
...
1. **`implement` vs `ship`.** Both take work to code. `implement` runs in-session
   on the current branch with no isolation; `ship` runs an autonomous
...
resolves by accident of what happens to be installed. Every skill here is one its
owner has an opinion about. And the specification holds: delete everything but
